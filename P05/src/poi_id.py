...
def evaluate_classifier(classifier):
    ...
    return (
        precision_score(labels_test, predictions, pos_label=1),
        recall_score(labels_test, predictions, pos_label=1),
        f1_score(labels_test, predictions, pos_label=1),
        classifier
    )
...
print "--------------------------------------------------"
for (precision, recall, f1score, clf) in results:
    clf_name = clf.__class__.__name__
    print "%25s   %.2f   %.2f   %.2f"%(clf_name,precision, recall, f1score)
print "--------------------------------------------------"

best_model_index = np.argmax(zip(*results)[1])
precision,max_recall,f1score, clf = results[best_model_index]
ordinal = lambda n: "%d%s" % (n,"tsnrhtdd"[(n/10%10!=1)*(n%10<4)*n%10::4])
print 'Selected %s model:'%(ordinal(best_model_index+1))
print "%s with recall = %.2f"%( clf.__class__.__name__, recall)
